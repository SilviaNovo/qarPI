...
#'
#' @details
#' This function implements the QAR-perc algorithm described in
...
#'
#'@export
pi_QAR_perc <- function(series, p = 1, h = 3, B = 1000, alpha = 0.05) {
  ...
  for (b in 1:B) {
    ...
    w <- stats::rexp(n - p)  # multipliers
    x.fut <- numeric(p + h)
    x.fut[1:p] <- series[(n - p + 1):n]

    for (t in 1:h) {
      q <- stats::runif(1)
      m.quantile.star <- quantreg::rq(y ~ xp, tau = q, weights = w)
